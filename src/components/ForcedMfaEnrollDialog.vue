<template>
  <q-dialog
    ...
    :data-testid="forcedMfaEnrollTestIds.dialog"
      ...
      <q-card-section class="forced-mfa-enroll-dialog__body">
        ...
        <q-form
          ...
          <img
            ...
            :data-testid="forcedMfaEnrollTestIds.qr"
          />
          <p
            v-if="secret"
            class="forced-mfa-enroll-dialog__secret"
            :data-testid="forcedMfaEnrollTestIds.secret">
            {{ secret }}
          </p>
          <LoginTextInput
            v-model="code"
            icon-left="pin"
            :test-id="forcedMfaEnrollTestIds.code"
          ...
          <q-btn
            ...
            class="full-width app-btn-primary
              forced-mfa-enroll-dialog__submit"
            :loading="verifying"
            :data-testid="forcedMfaEnrollTestIds.submit"
          ...
          <q-btn
            ...
            flat
            class="full-width"
            :data-testid="forcedMfaEnrollTestIds.cancel"
            :label="t('cancel')"
            @click="emit('close')"
          />
        </q-form>
        ...
        <div v-else class="forced-mfa-enroll-dialog__form">
          ...
          <q-btn
            ...
            class="full-width app-btn-primary
              forced-mfa-enroll-dialog__submit"
            :data-testid="forcedMfaEnrollTestIds.continue"
            :label="t('continue')"
            @click="onContinue"
          />
        </div>
...
import { forcedMfaEnrollTestIds } from 'src/test-ids/index.js'
...
const codeErrorMessage = computed(() =>
  code.value.trim() ? '' : t('loginMfaCodeRequired'),
)
...
async function onVerify() {
  submitError.value = ''
  isCodeInvalid.value = !!codeErrorMessage.value
